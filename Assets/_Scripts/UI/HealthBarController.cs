...
public class HealthBarController : MonoBehaviour
{
    ...
    void Start()
    {
        var charState = _fighter.GetComponent<CharacterState>();
        if (charState != null)
        {
            charState.OnHPChanged += UpdateBar;
        }
    }
    ...
    void UpdateBar(int current, int max)
    {
        ...
        _fillImage.color = pct > 0.5f ? _green : pct > 0.3f ? _yellow : _red;
    ...
    void Update()
    {
        if (_ghostTimer > 0)
        {
            ...
            if (_ghostTimer <= 0) _ghostSlider.value = _fillSlider.value;
        }
    }
}