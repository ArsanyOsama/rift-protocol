...
public class ArenaManager : MonoBehaviour
{
    ...
    {
        float dist = FighterDistance;
        if (dist >= _minSeparation) return;   // already far enough apart

        // How much total separation is needed
        float overlap = _minSeparation - dist;
        float halfPush = overlap * 0.5f;

        // Determine which direction to push each fighter
        // Fighter on the left gets pushed further left, fighter on right gets pushed right
        Vector2 posA = _fighterA.Position;
        Vector2 posB = _fighterB.Position;

        float dirA = posA.x <= posB.x ? -1f : 1f;   // A is left → push left
        float dirB = -dirA;                           // B is always opposite

        // Apply separation — but clamp to stage walls so we don't push through them
        float newAX = Mathf.Clamp(posA.x + dirA * halfPush, _stageLeft, _stageRight);
        float newBX = Mathf.Clamp(posB.x + dirB * halfPush, _stageLeft, _stageRight);

        _fighterA.Warp(new Vector2(newAX, posA.y));
        _fighterB.Warp(new Vector2(newBX, posB.y));
